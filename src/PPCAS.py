...
def main(sc, seq, to_file, n_partitions, m_ip, hdfs_path):
	...
	if(to_file==1):
		rdd_tasks.map(ProbaMatrix2CL).saveAsTextFile("hdfs://" + m_ip + ":9000" + hdfs_path + "/" + out_name)
	elif(to_file==2):
		tc_header(out_name, seq_list)
		hdfs_cmd(out_name)
		rdd_tasks.map(ProbaMatrix2CL).saveAsTextFile("hdfs://" + m_ip + ":9000" + hdfs_path + "/" + out_name)
	else: rdd_tasks.map(ProbaMatrix2CL).count()
	del seq_list[:]
...
if __name__ == "__main__":
	...
	main(sc, seq, to_file, n_partitions, m_ip, hdfs_path)